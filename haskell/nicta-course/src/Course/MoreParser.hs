...
tok pA = (\a -> spaces >>> valueParser a) =<< pA
...
-- True
hex :: Parser Char
hex = chr . hexInt <$> replicateA 4 (satisfy isHexDigit)
    where
        hexInt i = case readHex i of
            Full hI -> hI
            Empty -> 0
...
sepby1 pA pS = (\a ->
        (\as -> valueParser (a:.as)) =<< list (pS >>> pA)
    ) =<< pA
...
-- True
eof :: Parser ()
eof = P (\input -> if length input == 0 then Result Nil () else ErrorResult UnexpectedEof)
...
-- satisfyAll fs = flbindParser character (\c ->
--     if all (\f -> f c) fs then valueParser c else failed)
