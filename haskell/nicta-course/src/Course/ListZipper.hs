...
fromList Nil = IsNotZ
fromList (a:.r) = IsZ (ListZipper Nil a r)
...
findLeft p (ListZipper l v r) = case break p l of
    (_, Nil) -> IsNotZ
    (rs', v':.ls') -> IsZ $ ListZipper ls' v' (reverse rs' ++ v:.Nil ++ r)
...
findRight p (ListZipper l v r) = case break p r of
    (_, Nil) -> IsNotZ
    (ls', v':.rs') -> IsZ $ ListZipper (reverse ls' ++ v:.l) v' rs'
...
moveLeftLoop (ListZipper l v r) = case l of
    (v':.l') -> ListZipper l' v' (v:.r)
    Nil -> let (v':.r') = reverse r
        in ListZipper (r' ++ v:.Nil) v' Nil
...
moveRightLoop (ListZipper l v r) = case r of
    (v':.r') -> ListZipper (v:.l) v' r'
    Nil -> let (v':.l') = reverse l
        in ListZipper Nil v' (l' ++ v:.Nil)
...
moveLeft (ListZipper l v r) = case l of
    Nil -> IsNotZ
    (v':.l') -> IsZ $ ListZipper l' v' (v:.r)
...
moveRight (ListZipper l v r) = case r of
    Nil -> IsNotZ
    (v':.r') -> IsZ $ ListZipper (v:.l) v' r'
...
swapLeft (ListZipper l v r) = case l of
    Nil -> IsNotZ
    (v':.l') -> IsZ $ ListZipper (v:.l') v' r
...
swapRight (ListZipper l v r) = case r of
    Nil -> IsNotZ
    (v':.r') -> IsZ $ ListZipper l v' (v:.r')
...
moveLeftN n lz
    | n == 0 = IsZ lz
    | n < 0 = moveRightN (-n) lz
    | otherwise = withZipper (moveLeft lz) (moveLeftN (n - 1))
    where
        withZipper IsNotZ _ = IsNotZ
        withZipper (IsZ z) next = next z
...
moveRightN n lz
    | n == 0 = IsZ lz
    | n < 0 = moveLeftN (-n) lz
    | otherwise = withZipper (moveRight lz) (moveRightN (n - 1))
    where
        withZipper IsNotZ _ = IsNotZ
        withZipper (IsZ z) next = next z
...
start (ListZipper l v r) =
    let (v':.l') = reverse l
    in ListZipper Nil v' (l' ++ v:.r)
...
-- /Tip:/ Use `zipWith`
  ListZipper lf vf rf <*> ListZipper l v r = ListZipper (zipWith ($) lf l) (vf v) (zipWith ($) rf r)
...
instance Extend ListZipper where
  f <<= z = ListZipper (unfoldr (unfold moveLeft) z) (f z) (unfoldr (unfold moveRight) z)
    where
        unfold move a = case move a of
            IsZ zm -> Full (f zm, zm)
            IsNotZ -> Empty
...
instance Traversable ListZipper where
  traverse f (ListZipper l v r) =
      (ListZipper . reverse) <$> traverse f (reverse l) <*> f v <*> traverse f r
...
instance Traversable MaybeListZipper where
  traverse f maybeZ = case maybeZ of
      IsNotZ -> pure IsNotZ
