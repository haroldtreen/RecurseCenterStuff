...
-- OR replicateA n = sequence . replicate n
...
filtering p = foldRight (\a fla -> lift3 select (p a) (pure a) fla) (pure Nil)
    where
        select b a la = if b then a:.la else la
-- OR filtering p = foldRight (\a -> lift2 (\b -> if b then (a:.) else id) (p a)) (pure Nil)
